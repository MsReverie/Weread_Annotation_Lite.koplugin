...
local Paginator = require("WereadAnnotationLite.ui.thought_popup_renderer").Paginator
...
function ScrollContainer:init()
    ...
    -- VerticalScrollBar normally initializes this during paintTo(). A touch
    -- can arrive before the first paint pass, so initialize it eagerly.
    self.v_scroll_bar.touch_dimen = Geom:new{
        x = 0, y = 0, w = self.scrollbar_w, h = self.viewport_h,
    }
...
function ScrollContainer:scrollToPage(delta)
    local pages = self.pages
    if not delta or delta == 0 or not pages or #pages < 2 then return end
    local cur = self:_currentPageIndex()
    local target = math.min(#pages, math.max(1, cur + delta))
    if target ~= cur then
        -- Use the logical page start directly. The final page may be shorter
        -- than the viewport and intentionally leaves blank space below it.
...
function ScrollContainer:onScrollText(arg, ges)
    -- Swipe up/north = forward, down/south = backward. Some KOReader
    -- input drivers report the short up/down names instead of compass names.
    if ges.direction == "north" or ges.direction == "up" then
        self:scrollToPage(1)
        return true
    elseif ges.direction == "south" or ges.direction == "down" then
        self:scrollToPage(-1)
        return true
    end
...
function ScrollContainer:paintTo(bb, x, y)
    ...
    if self.v_scroll_bar then
        ...
        self.v_scroll_bar.touch_dimen.h = self.viewport_h
